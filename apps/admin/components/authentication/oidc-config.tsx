...
export const OIDCConfiguration = observer(function OIDCConfiguration(props: Props) {
  ...
  // The issuer is what every endpoint is discovered from, so it is required
  // alongside the client credentials before the method can be switched on.
    ...
    <>
      {isOIDCConfigured ? (
        <div className="flex items-center gap-4">
          ...
          />
          <Switch
            checked={Boolean(parseInt(enableOIDCConfig))}
            onCheckedChange={() => {
