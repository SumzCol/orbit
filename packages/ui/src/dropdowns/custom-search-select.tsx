      ...
      {({ open }: { open: boolean }) => {
        ...
        return (
          <>
            ...
            {isOpen &&
              /* Combobox.Options is both the popper element and the panel, as in
                 CustomSelect. React does not attach a ref to a plain div rendered here,
                 so with the ref one level in usePopper had no popper element and left
                 the panel at its initial top-left. The panel classes come up with it:
                 z-index does nothing on a static box, and a margin below the positioned
                 element pads the panel instead of offsetting it from the trigger. */
              createPortal(
                <Combobox.Options
                  as="ul"
